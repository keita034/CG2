...
//パイプラインステート
Microsoft::WRL::ComPtr<ID3D12PipelineState> Sprite2D::pipelineState = nullptr;
//ルートシグネチャ
ID3D12RootSignature* Sprite2D::rootSignature = nullptr;
//SRVの最大個数
const size_t Sprite2D::kMaxSRVCount = 2056;
//デスクプリタヒープ
ID3D12DescriptorHeap* Sprite2D::srvHeap;
//デスクプリタレンジ
D3D12_DESCRIPTOR_RANGE Sprite2D::descriptorRange{};
//スプライト数
UINT Sprite2D::nextIndex = 0;

//パイプラインの生成
void Sprite2D::CreatPipeline()
{
	...
	result = D3DCompileFromFile(
		L"Resources/Shaders/2D/Sprite2DVS.hlsl", // シェーダファイル名
		nullptr,
	...
	// ピクセルシェーダの読み込みとコンパイル
	result = D3DCompileFromFile(
		L"Resources/Shaders/2D/Sprite2DPS.hlsl", // シェーダファイル名
		nullptr,
		D3D_COMPILE_STANDARD_FILE_INCLUDE, // インクルード可能にする
...
//デスクプリタヒープの生成
void Sprite2D::CreateDescriptorHeap()
{
	// デスクリプタヒープの設定
	...
	result = device->CreateDescriptorHeap(&srvHeapDesc, IID_PPV_ARGS(&Sprite2D::srvHeap));
	assert(SUCCEEDED(result));
}

//デスクプリタレンジの生成
void Sprite2D::CreateDescriptorRange()
{
...
void Sprite2D::CreatConstBuff()
{
	...
	MyMath::MakeOrthogonalL(0.0f, width, height, 0.0f, 0.0f, 1.0f, projection);
}
...
//コンストラクタ
Sprite2D::Sprite2D(const wchar_t* filePath)
{
...
void Sprite2D::Update(const MyMath::Vector3& pos, const MyMath::Vector3& rot, const MyMath::Vector3& scale, const MyMath::Vector4& color)
{
	...
	constMapTransform->matWorld = projection * world;
}
