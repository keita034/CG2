...
void Sprite3D::Load(const wchar_t* filePath)
{
	...
	//画像の半分のサイズ
	float halfW = static_cast<float>(metadata.width) * 0.0625f;
	float halfH = static_cast<float>(metadata.height) * 0.0625f;
...
void Sprite3D::BillboardDraw(Camera* camera)
{
	assert(camera);

	MyMath::Matrix4 mat = camera->GetViewMatrix();
	mat.m[3][0] = 0;
	mat.m[3][1] = 0;
	mat.m[3][2] = 0;
	mat.m[3][3] = 1;
...
void Sprite3D::BillboardXDraw(Camera* camera)
{
	assert(camera);
...
void Sprite3D::BillboardYDraw(Camera* camera)
{
	assert(camera);
...
void Sprite3D::BillboardZDraw(Camera* camera)
{
	assert(camera);
