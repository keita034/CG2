...
Microsoft::WRL::ComPtr<ID3D12PipelineState> ModelCreate::objPipelineState;
Microsoft::WRL::ComPtr<ID3D12RootSignature> ModelCreate::objRootSignature;

Model* ModelCreate::CreateModel(ModelShape shape, const wchar_t* filePath)
{
...
Model* ModelCreate::CreateModel(const char* filePath)
{
	if (modelShareVaria.srvHeap == nullptr)
	{
		//デスクプリタヒープ生成
		CreateDescriptorHeap();
	...
	if (objPipelineState == nullptr)
	{
		//パイプライン生成
		CreatObjPipeline();
	}
	
	Model* cube = new objModel;
	cube->Initialize(modelShareVaria,objPipelineState.Get(), objRootSignature.Get());
	cube->Create(filePath);
	modelShareVaria.nextIndex++;
	return cube;
...
void ModelCreate::CreatObjPipeline()
{
	HRESULT result;
	Microsoft::WRL::ComPtr<ID3D12Device> device = DirectX12Core::GetInstance()->GetDevice();
	...
	rootParams[1].Descriptor.ShaderRegister = 1;//定数バッファバッファ番号
	rootParams[1].Descriptor.RegisterSpace = 0;//デフォルト値
	rootParams[1].ShaderVisibility = D3D12_SHADER_VISIBILITY_ALL;//全てのシェーダーから見える

	//テクスチャレジスタ0番
	rootParams[2].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;//種類
	rootParams[2].DescriptorTable.pDescriptorRanges = &modelShareVaria.descriptorRange;//デスクプリタレンジ
	rootParams[2].DescriptorTable.NumDescriptorRanges = 1;//デスクプリタレンジ数
	...
	assert(SUCCEEDED(result));
	// パイプラインにルートシグネチャをセット
	pipelineDesc.pRootSignature = objRootSignature.Get();

	// パイプランステートの生成
	result = device->CreateGraphicsPipelineState(&pipelineDesc, IID_PPV_ARGS(objPipelineState.ReleaseAndGetAddressOf()));
	assert(SUCCEEDED(result));
}
